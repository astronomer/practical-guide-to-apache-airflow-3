...
def basehook_get_connection_monkeypatch(
    key: str, *args, **kwargs
):
    print(
...
BaseHook.get_connection = (
    basehook_get_connection_monkeypatch
)
# # =========== /MONKEYPATCH BASEHOOK.GET_CONNECTION() ===========
...
def os_getenv_monkeypatch(key: str, *args, **kwargs):
    ...
    if args:
        default = args[
            0
        ]  # os.getenv should get at most 1 arg after the key
    if kwargs:
        default = kwargs.get(
            "default", None
...
def test_file_imports(rel_path, rv):
    ...
    if os.path.exists(
        ...
        with open(
            ".astro/dag_integrity_exceptions.txt", "r"
        ) as f:
            exceptions = f.readlines()
    print(f"Exceptions: {exceptions}")
    if (
        rv != "No import errors"
    ) and rel_path not in exceptions:
